...
public class SkuDetails implements Parcelable {
    ...
    public List<Pair<String, String>> outputList() {
        final List<Pair<String, String>> outputList = new ArrayList<>();
        outputList.add(
                new Pair<>("عنوان محصول :",
                        String.format(App.Companion.getIranLocale(), "%s", title)
                )
        );
        outputList.add(
                new Pair<>("توضیحات محصول :",
                        String.format(App.Companion.getIranLocale(), "%s", TextUtils.isEmpty(description) ? "خالی" : description)
                )
        );
        outputList.add(
                new Pair<>("قیمت محصول :",
                        String.format(App.Companion.getIranLocale(), "%s", priceText)
                )
        );
        return outputList;
    }
