 ...
 */
package farmani.masoud.app.traffic_regu_iab.iab;
...
public class BillingProcessor extends BillingBase {
    ...
    private BillingProcessor(Context context, String licenseKey, String merchantId, IBillingHandler handler,
        ...
        if (!isCafeBazaarInstalled()) {
            reportBillingError(Constants.CAFE_BAZAAR_NOT_INSTALLED, new Throwable("کافه بازار نصب نیست"));
            return;
        }
    ...
    private boolean isCafeBazaarInstalled() {
    ...
    private void bindCafeBazaarPardakhtService() {
        try {
            getContext().bindService(getBindServiceIntent(), serviceConnection, Context.BIND_AUTO_CREATE);
        } catch (Exception e) {
            LogCat.Companion.log(LOG_TAG, "error in bindCafeBazaarPardakhtService", 'e', e);
    ...
    public void release() {
        if (isInitialized() && serviceConnection != null) {
            try {
                ...
                LogCat.Companion.log(LOG_TAG, "Error in release", 'e', e);
            }
            billingService = null;
        }
    }
    ...
    private boolean loadPurchasesByType(String type, BillingCache cacheStorage) {
        ...
        try {
            ...
            LogCat.Companion.log(LOG_TAG, "Error in loadPurchasesByType", 'e', e);
        }
        return false;
    }
     ...
     */
    public boolean loadOwnedPurchasesFromCafeBazaar() {
    ...
    public boolean isOneTimePurchaseSupported() {
        if (!isInitialized()) {
            LogCat.Companion.log(
                    LOG_TAG,
                    "Make sure BillingProcessor was initialized before calling isOneTimePurchaseSupported()",
                    'e');
            return false;
        }
    ...
    private boolean purchase(Activity activity, List<String> oldProductIds, String productId,
        ...
        try {
            ...
            if (bundle != null) {
                ...
                if (response == Constants.BILLING_RESPONSE_RESULT_OK) {
                    ...
                    if (!isPurchased(productId) && !isSubscribed(productId)) {
                        loadOwnedPurchasesFromCafeBazaar();
                    }
                    TransactionDetails details = getPurchaseTransactionDetails(productId);
                    if (!checkMerchant(details)) {
                        LogCat.Companion.log(LOG_TAG, "Invalid or tampered merchant id!", 'e');
            ...
            LogCat.Companion.log(LOG_TAG, "Error in purchase", 'e', e);
        ...
        try {
            ...
            if (transaction != null && !TextUtils.isEmpty(transaction.purchaseToken)) {
                ...
                if (response == Constants.BILLING_RESPONSE_RESULT_OK) {
                    ...
                    LogCat.Companion.log(LOG_TAG, "Successfully consumed " + productId + " purchase.", 'e');
                    return true;
                } else {
                    reportBillingError(response, null);
                    LogCat.Companion.log(LOG_TAG, String.format("Failed to consume %s: %d", productId, response), 'e');
                }
            }
        } catch (Exception e) {
            LogCat.Companion.log(LOG_TAG, "Error in consumePurchase", 'e', e);
        ...
        if (billingService != null && productIdList != null && productIdList.size() > 0) {
            try {
                ...
                if (response == Constants.BILLING_RESPONSE_RESULT_OK) {
                    ...
                    reportBillingError(response, null);
                    LogCat.Companion.log(LOG_TAG, String.format("Failed to retrieve info for %d products, %d",
                            productIdList.size(),
                            response), 'e');
                }
            } catch (Exception e) {
                LogCat.Companion.log(LOG_TAG, "Failed to call getSkuDetails", 'e', e);
    ...
    public boolean handleActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (data == null) {
            LogCat.Companion.log(LOG_TAG, "handleActivityResult: data is null!", 'e');
            return false;
        }
        int responseCode = data.getIntExtra(Constants.RESPONSE_CODE, Constants.BILLING_RESPONSE_RESULT_OK);
        LogCat.Companion.log(LOG_TAG, String.format("resultCode = %d, responseCode = %d", resultCode, responseCode), 'e');
        if (resultCode == Activity.RESULT_OK &&
            ...
            try {
                ...
                if (verifyPurchaseSignature(productId, purchaseData, dataSignature)) {
                    ...
                    LogCat.Companion.log(LOG_TAG, "Public key signature doesn't match!", 'e');
                    reportBillingError(Constants.BILLING_ERROR_INVALID_SIGNATURE, null);
                }
            } catch (Exception e) {
                LogCat.Companion.log(LOG_TAG, "Error in handleActivityResult", 'e', e);
                reportBillingError(Constants.BILLING_ERROR_OTHER_ERROR, e);
            }
    ...
    private class HistoryInitializationTask extends AsyncTask<Void, Void, Boolean> {
        ...
        protected Boolean doInBackground(Void... nothing) {
            if (!isPurchaseHistoryRestored()) {
                return loadOwnedPurchasesFromCafeBazaar();
            }
            return false;
        }
